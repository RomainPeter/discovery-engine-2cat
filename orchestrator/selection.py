...
class BanditSelector:
    ...
    async def select_bandit(self,
                          state: Dict[str, Any],
                          diversity_scores: Dict[str, float],
                          budget: Dict[str, float]) -> Dict[str, List[str]]:
        """
    ...
    def _extract_features(self, state, diversity_scores: Dict[str, float]) -> np.ndarray:
        ...
        if hasattr(state, 'X') and isinstance(state.X, dict):
            budget_utilization = sum(state.X.get("budget_used", {}).values()) / sum(state.X.get("budget", {}).values()) if state.X.get("budget", {}) else 0
        else:
            budget_utilization = sum(state.get("budget_used", {}).values()) / sum(state.get("budget", {}).values()) if state.get("budget", {}) else 0
        features.append(budget_utilization)
        
        return np.array(features)
    
    async def _select_arms(self,
                         arms: List[str],
                         features: np.ndarray,
                         arm_type: str) -> List[str]:
...
class MCTSSelector:
    ...
    def _create_root_node(self, state: Dict[str, Any], diversity_scores: Dict[str, float], budget: Dict[str, float]) -> str:
    ...
    def _get_best_actions(self, root_id: str) -> Dict[str, List[str]]:
        ...
        implications = [action["id"] for action in selected_actions if action["type"] == "implication"]
        choreographies = [action["id"] for action in selected_actions if action["type"] == "choreography"]
