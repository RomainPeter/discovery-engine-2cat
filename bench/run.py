...
class BenchmarkRunner:
    ...
    def run_suite(self, suite: str, baselines: List[str], ablations: List[str]) -> Dict[str, Any]:
    ...
    def _calculate_metrics(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for baseline, baseline_results in results["baselines"].items():
            ...
            ) / baseline_metrics["coverage_gain"] * 100
            
            cost_improvement = (
                baseline_metrics["audit_cost_ms"] - de_metrics["audit_cost"]["simulated_ms"]
            ) / baseline_metrics["audit_cost_ms"] * 100
        ...
        for ablation, ablation_results in results["ablations"].items():
            ...
            ) / de_metrics["coverage_gain"] * 100
            
            ablation_impacts[ablation] = {
                "coverage_impact_pct": coverage_impact
            }
    ...
    def _check_go_no_go(self, metrics: Dict[str, Any], improvements: Dict[str, Any]) -> Dict[str, bool]:
        """Check Go/No-Go criteria"""
        criteria = {
            "coverage_gain_20pct": any(
                imp["coverage_improvement_pct"] >= 20 
                for imp in improvements.values()
            ),
            "audit_cost_15pct": any(
                imp["cost_improvement_pct"] >= 15 
                for imp in improvements.values()
            ),
...
def main():
    parser = argparse.ArgumentParser(description="Discovery Engine 2-Cat Benchmark Suite")
    parser.add_argument("--suite", default="regtech", help="Benchmark suite to run")
    parser.add_argument("--baselines", nargs="+", default=["react", "tot", "dspy"], 
                       help="Baselines to compare against")
    parser.add_argument("--ablations", nargs="+", default=["egraph", "bandit", "dpp", "incident"],
