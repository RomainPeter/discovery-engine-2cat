...
class EGraph:
            ...
            if rule.pattern(node):
                ...
                if rule.guard(node, context):
                    ...
                    for existing_id, existing_node in self.nodes.items():
                        if (existing_node.operator == node.operator and 
                            self._nodes_equivalent(node, existing_node, rule)):
    ...
    def _deep_equivalent(self, obj1: Any, obj2: Any) -> bool:
        ...
        if isinstance(obj1, dict):
            return (set(obj1.keys()) == set(obj2.keys()) and
                    all(self._deep_equivalent(obj1[k], obj2[k]) for k in obj1.keys()))
    ...
    def _is_retrieve_normalize_chain(self, node: ENode) -> bool:
        ...
        return (node.metadata.get("composite", False) and
                "retrieve" in str(node.metadata.get("ops", [])) and
                "normalize" in str(node.metadata.get("ops", [])))
    ...
    def _is_meet_chain(self, node: ENode) -> bool:
        ...
        return (node.operator == OperatorType.MEET and
                len(node.children) >= 2)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_class_size": sum(len(c.members) for c in self.classes.values()) / len(self.classes) if self.classes else 0,
